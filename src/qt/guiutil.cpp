...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PENG address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no PENG: URI
...
{
    // Convert peng:// to peng:
    //
    //    Cannot handle this later, because peng:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open peng.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PENG.lnk";
}
...
{
    return GetAutostartDir() / "peng.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a peng.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PENG\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the peng app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add peng app to startup item list
