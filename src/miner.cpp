...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PENGMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PENGMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PENGMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("peng-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PENGMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
