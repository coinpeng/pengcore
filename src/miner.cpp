...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PenguinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PenguinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PenguinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("penguin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1471482000 || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || (pwallet->GetBalance() > 5000 && nReserveBalance >= pwallet->GetBalance()) || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
        ...
        LogPrintf("Running PenguinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
